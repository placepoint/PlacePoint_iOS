...
open class TableCell: UITableViewCell {
  ...
  func makeLabel() -> UILabel {
    ...
    label.textColor = Config.List.DefaultCell.Text.lblCellColor
    
    label.textAlignment = .center
    
    label.frame = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.size.width - 120 , height: contentView.frame.size.height)
  ...
  func makeCheckmark() -> UIImageView {
    ...
    let checkmark = UIImageView(image: AssetManager.image("")?.withRenderingMode(.alwaysTemplate))

//    checkmark.tintColor = Config.List.DefaultCell.Text.lblCellColor
