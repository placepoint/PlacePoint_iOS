...
+ (void)initialize {
	if (self == [FRHyperLabel class]) {
		FRHyperLabelLinkColorDefault = [UIColor colorWithRed:54/255.0 green:131/255.0 blue:251/255.0 alpha:1];
...
- (void)setLinkForSubstring:(NSString *)substring withAttribute:(NSDictionary *)attribute andLinkHandler:(void(^)(FRHyperLabel *label, NSString *substring))handler {
	...
	if (range.length) {
        ...
        NSDictionary *attributes1 = @{NSForegroundColorAttributeName: [UIColor colorWithRed:54/255.0 green:131/255.0 blue:251/255.0 alpha:1],NSFontAttributeName: [UIFont preferredFontForTextStyle:UIFontTextStyleHeadline]};
        
		[self setLinkForRange:range withAttributes:attributes1 andLinkHandler:^(FRHyperLabel *label, NSRange range){
